...
/// <summary>
/// V2 External module for covered_transactions.
/// Tier 3 (Full External): queries PostgreSQL directly because DataSourcing cannot express
/// snapshot fallback (unbounded lower date range) and SQLite lacks DISTINCT ON.
/// Produces a denormalized view of transactions for Checking account holders with active US addresses.
/// </summary>
public class CoveredTransactionsV2Processor : IExternalStep
{
    // --- Named constants (AP7 elimination) ---

    /// <summary>Only Checking accounts qualify for covered transactions.</summary>
    private const string CoveredAccountType = "Checking";

    /// <summary>Only customers with an active US address are included.</summary>
    private const string RequiredCountry = "US";
    ...
    public Dictionary<string, object> Execute(Dictionary<string, object> sharedState)
    {
        ...
        // 1. Fetch transactions for the effective date (exact date match)
        var transactions = FetchRows(connection, TransactionsQuery, dateParam);

        // 2. Fetch accounts with snapshot fallback (most recent on or before effective date)
        var accountRows = FetchRows(connection, AccountsSnapshotQuery, dateParam);

        // Build dictionary of Checking accounts only, keyed by account_id
        ...
        var customerRows = FetchRows(connection, CustomersSnapshotQuery, dateParam);
        ...
        // 4. Fetch active US addresses (first per customer = earliest start_date due to ORDER BY)
        var addressRows = FetchRows(connection, ActiveUsAddressesQuery, dateParam);
        ...
        foreach (var row in addressRows)
        {
            ...
            activeUsAddresses.TryAdd(customerId, row);
        }

        // 5. Fetch segment mappings (first alphabetical segment_code per customer via DISTINCT ON)
        var segmentRows = FetchRows(connection, SegmentsQuery, dateParam);
        ...
        // 6. Join: transaction -> checking account -> customer with active US address
    ...
    /// <summary>
    /// Execute a parameterized query and return results as a list of dictionaries.
    /// </summary>
    ...
    /// <summary>
    /// Format a timestamp value as "yyyy-MM-dd HH:mm:ss".
    /// </summary>
    private static string? FormatTimestamp(object? value)
    {
        if (value is null || value is DBNull) return null;
        if (value is DateTime dt) return dt.ToString(TimestampFormat);
